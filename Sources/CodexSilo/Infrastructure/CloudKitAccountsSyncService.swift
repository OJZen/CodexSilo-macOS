...
struct CloudSyncAvailabilityService: CloudSyncAvailabilityServiceProtocol {
    private let containerIdentifier = "iCloud.com.alick.codexsilo"
...
actor CloudKitAccountsSyncService: AccountsCloudSyncServiceProtocol {
    private enum Constants {
        static let containerIdentifier = "iCloud.com.alick.codexsilo"
...
actor CloudKitCurrentAccountSelectionSyncService: CurrentAccountSelectionSyncServiceProtocol {
    private enum Constants {
        ...
        static let schemaVersion = 1
        static let deviceIDDefaultsKey = "codexsilo.current-selection.device-id"
    }
    ...
    func recordLocalSelection(accountID: String) async throws {
        let store = try storeRepository.loadStore()
        guard let selectedAccount = resolveSelectedAccount(in: store, selectionIdentifier: accountID) else {
        ...
        #if DEBUG
        debugSelectionLog("recorded local selection selectionKey=\(selection.resolvedVariantKey ?? selection.resolvedAccountKey) selectedAt=\(selection.selectedAt) sourceDeviceID=\(selection.sourceDeviceID)")
        #endif
    }
    ...
    func pushLocalSelectionIfNeeded() async throws {
        ...
        guard selectionDigest != lastUploadedDigest else {
            #if DEBUG
            debugSelectionLog("push skipped; local selection digest already uploaded for selectionKey=\(selection.resolvedVariantKey ?? selection.resolvedAccountKey)")
            #endif
            return
        }
        ...
        if pushedSelection == selection {
            ...
            #if DEBUG
            debugSelectionLog("push succeeded for selectionKey=\(selection.resolvedVariantKey ?? selection.resolvedAccountKey) selectedAt=\(selection.selectedAt)")
            #endif
        } else {
            #if DEBUG
            debugSelectionLog(
                "push resolved to server selection selectionKey=\(pushedSelection.resolvedVariantKey ?? pushedSelection.resolvedAccountKey) selectedAt=\(pushedSelection.selectedAt) sourceDeviceID=\(pushedSelection.sourceDeviceID)"
            )
            #endif
        }
    }
    ...
    func pullRemoteSelectionIfNeeded() async throws -> CurrentAccountSelectionPullResult {
        ...
        if remoteDigest == lastAppliedDigest {
            #if DEBUG
            debugSelectionLog("pull skipped; remote selection already applied for selectionKey=\(remoteSelection.resolvedVariantKey ?? remoteSelection.resolvedAccountKey)")
            #endif
            return .noChange
        }
        ...
        if !CloudKitSelectionMerge.shouldApplyRemoteSelection(
            ...
            #if DEBUG
            debugSelectionLog(
                "pull ignored; local selection wins over remote selectionKey=\(remoteSelection.resolvedVariantKey ?? remoteSelection.resolvedAccountKey) selectedAt=\(remoteSelection.selectedAt)"
            )
            #endif
            return .noChange
        }

        guard let matchingAccount = resolveSelectedAccount(in: store, selection: remoteSelection) else {
            #if DEBUG
            debugSelectionLog(
                "pull ignored; remote selected selectionKey=\(remoteSelection.resolvedVariantKey ?? remoteSelection.resolvedAccountKey) not found in local store"
            )
            #endif
            return .noChange
        }

        let appliedCurrentSelection = localAppliedCurrentSelectionIdentifiers(fallingBackTo: previousSelection)
        let changedCurrentAccount = !matchingAccount.matchesSelection(
            accountKey: appliedCurrentSelection.accountKey,
            variantKey: appliedCurrentSelection.variantKey
        )
        if runtimePlatform == .macOS, changedCurrentAccount {
            try authRepository.writeCurrentAuth(matchingAccount.authJSON)
        }

        let normalizedSelection = CurrentAccountSelection(
            accountID: matchingAccount.accountID,
            accountKey: matchingAccount.accountKey,
            variantKey: matchingAccount.variantKey,
            selectedAt: remoteSelection.selectedAt,
            sourceDeviceID: remoteSelection.sourceDeviceID
        )
        try saveCurrentSelection(normalizedSelection)
        lastAppliedDigest = remoteDigest
        lastUploadedDigest = remoteDigest
        #if DEBUG
        debugSelectionLog(
            "pull applied remote selection selectionKey=\(matchingAccount.variantKey) changedCurrentAccount=\(changedCurrentAccount) previousSelectionKey=\(previousSelection?.resolvedVariantKey ?? previousSelection?.resolvedAccountKey ?? "<nil>")"
        )
        #endif
        return CurrentAccountSelectionPullResult(
            didUpdateSelection: changedCurrentAccount || previousSelection != normalizedSelection,
    ...
    private func synchronizeLocalSelectionMetadata() throws -> CurrentAccountSelection? {
        ...
        if let existing = store.currentSelection,
           let matchingAccount = resolveSelectedAccount(in: store, selection: existing) {
            let normalized = CurrentAccountSelection(
                accountID: matchingAccount.accountID,
                accountKey: matchingAccount.accountKey,
                variantKey: matchingAccount.variantKey,
            ...
            if normalized != existing {
                try saveCurrentSelection(normalized)
            }
            #if DEBUG
            debugSelectionLog("using stored local selection selectionKey=\(normalized.resolvedVariantKey ?? normalized.resolvedAccountKey) selectedAt=\(normalized.selectedAt)")
            #endif
            return normalized
        }

        let currentAuthSelection = (
            accountKey: authRepository.currentAuthAccountKey(),
            variantKey: authRepository.currentAuthVariantKey()
        )
        guard runtimePlatform == .macOS,
              let matchingAccount = resolveSelectedAccount(in: store, selectionIdentifiers: currentAuthSelection) else {
            return nil
        }
        ...
        #if DEBUG
        debugSelectionLog("inferred local selection from auth selectionKey=\(matchingAccount.variantKey)")
        #endif
        return inferred
    }

    private func localAppliedCurrentSelectionIdentifiers(
        fallingBackTo selection: CurrentAccountSelection?
        ...
        if runtimePlatform == .macOS {
            ...
            if currentAccountKey?.isEmpty == false || currentVariantKey?.isEmpty == false {
                return (
                    currentAccountKey ?? selection?.resolvedAccountKey,
                    currentVariantKey ?? selection?.resolvedVariantKey
                )
            }
        }
    ...
    private func resolveSelectedAccount(
        ...
        if let matchingVariant = store.accounts.first(where: { $0.variantKey == selectionIdentifier }) {
            return matchingVariant
        }

        let matchingAccounts = store.accounts.filter { $0.accountKey == selectionIdentifier }
        if matchingAccounts.count == 1 {
            return matchingAccounts[0]
        }
        if let unknownPlanMatch = matchingAccounts.first(where: {
            AccountIdentity.normalizePlanTypeKey($0.resolvedPlanType) == "unknown"
        }) {
            return unknownPlanMatch
        }

        let matchingLegacyAccounts = store.accounts.filter { $0.accountID == selectionIdentifier }
        if matchingLegacyAccounts.count == 1 {
            return matchingLegacyAccounts[0]
        }
    ...
    private func resolveSelectedAccount(
        in store: AccountsStore,
        selectionIdentifiers: (accountKey: String?, variantKey: String?)
    ) -> StoredAccount? {
        if let variantKey = selectionIdentifiers.variantKey,
           let matchingVariant = store.accounts.first(where: { $0.variantKey == variantKey }) {
            return matchingVariant
        }

        if let accountKey = selectionIdentifiers.accountKey {
            return resolveSelectedAccount(in: store, selectionIdentifier: accountKey)
        }
...
enum CloudKitAccountsStoreMerge {
    ...
    static func applyingRemoteSnapshot(
        ...
        let localAccountsByVariantKey = latestStore.accounts.reduce(into: [String: StoredAccount]()) { partialResult, account in
            partialResult[account.variantKey] = account
        }
        let localUnknownVariantByAccountKey = latestStore.accounts.reduce(into: [String: StoredAccount]()) { partialResult, account in
            guard AccountIdentity.normalizePlanTypeKey(account.resolvedPlanType) == "unknown" else { return }
            partialResult[account.accountKey] = account
        }
        ...
        for remoteAccount in remoteAccounts {
            if let localAccount = localAccountsByVariantKey[remoteAccount.variantKey] {
                mergedAccounts.append(
                    mergeMatchedAccount(local: localAccount, remote: remoteAccount)
                )
                consumedAccountIDs.insert(localAccount.id)
            } else if AccountIdentity.normalizePlanTypeKey(remoteAccount.resolvedPlanType) != "unknown",
                      let placeholder = localUnknownVariantByAccountKey[remoteAccount.accountKey] {
                mergedAccounts.append(
                    mergeMatchedAccount(local: placeholder, remote: remoteAccount)
                )
                consumedAccountIDs.insert(placeholder.id)
            } else {
                mergedAccounts.append(remoteAccount)
            }
        }

        for localAccount in latestStore.accounts where !consumedAccountIDs.contains(localAccount.id) {
    ...
    private static func mergeMatchedAccount(local: StoredAccount, remote: StoredAccount) -> StoredAccount {
        ...
        merged.id = local.id
        merged.principalID = preferredMetadataValue(
            primary: metadataWinner.principalID,
            fallback: remoteMetadataWins ? local.principalID : remote.principalID
        )
...
enum CloudKitSelectionMerge {
    ...
    private static func comparesNewer(
        ...
        return (lhs.resolvedVariantKey ?? lhs.resolvedAccountKey) > (rhs.resolvedVariantKey ?? rhs.resolvedAccountKey)
    }
}
