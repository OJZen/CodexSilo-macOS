...
final class RemoteProxyService: RemoteProxyServiceProtocol, @unchecked Sendable {
    ...
    // private let logger = Logger(subsystem: "CodexSilo", category: "RemoteProxyService")
    ...
    private func buildRemoteAccountsStoreData() throws -> Data {
        ...
        if !mergedStore.accounts.contains(where: isProxyUsable(account:)),
           ...
           let imported = buildStoredAccount(fromCurrentAuth: auth) {
            if let index = mergedStore.accounts.firstIndex(where: { $0.variantKey == imported.variantKey }) {
                mergedStore.accounts[index] = imported
            } else if AccountIdentity.normalizePlanTypeKey(imported.resolvedPlanType) != "unknown",
                      let index = mergedStore.accounts.firstIndex(where: {
                          $0.accountKey == imported.accountKey
            ...
            loadDiagnostics.append("current_auth_imported=\(imported.variantKey)")
        }
    ...
    private func mergeAccounts(from incoming: [StoredAccount], into merged: inout AccountsStore) {
        for account in incoming {
            if let index = merged.accounts.firstIndex(where: { $0.variantKey == account.variantKey }) {
                merged.accounts[index] = preferredAccount(existing: merged.accounts[index], incoming: account)
            } else if AccountIdentity.normalizePlanTypeKey(account.resolvedPlanType) != "unknown",
                      let index = merged.accounts.firstIndex(where: {
    ...
    private func extractAuthFromJSONValue(_ auth: JSONValue) -> (principalID: String, accountID: String, email: String?, planType: String?)? {
        ...
        if let claims = try? decodeJWTPayload(idToken) {
            ...
            principalID = AccountIdentity.principalID(fromClaims: claims, email: email)
        }
    ...
    private func proxydBuildTargetDirectory() throws -> URL {
        ...
        let path = caches
            .appendingPathComponent("CodexSilo", isDirectory: true)
