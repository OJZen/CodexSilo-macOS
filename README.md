...
CodexSilo is a SwiftUI app for macOS that manages Codex/ChatGPT auth accounts, usage-based smart switching, and local API proxy workflows.

CodexSilo 是一个面向 macOS 的 SwiftUI 应用，用于管理 Codex/ChatGPT 授权账号、按用量智能切换，以及本地 API 代理工作流。
...
- 菜单栏集成（MenuBarExtra）
- Full main window opened from the menu bar icon
- 从菜单栏图标打开完整主窗口
...
Open `CodexSilo.xcodeproj` in Xcode and run the `CodexSilo` scheme for macOS.

使用 Xcode 打开 `CodexSilo.xcodeproj`，运行 `CodexSilo` scheme 即可。
...
- `Sources/CodexSilo/App`: scene composition and app bootstrap
- `Sources/CodexSilo/Features`: page-level composition and bindings
- `Sources/CodexSilo/UI`: reusable visual primitives
- `Sources/CodexSilo/Behavior`: coordinators and behavior modules
- `Sources/CodexSilo/Infrastructure`: IO/network/process integrations
- `Sources/CodexSilo/Domain`: models and protocols (single source of truth)
- `Sources/CodexSilo/Layout`: centralized layout rules
